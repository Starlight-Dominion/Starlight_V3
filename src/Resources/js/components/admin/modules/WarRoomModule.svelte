...
<div in:fade class="space-y-12">
    <div class="flex justify-between items-center">
        ...
        <button onclick={onAdd} class="bg-white text-black font-title font-black text-[10px] px-8 py-3 rounded-xl hover:bg-cyan-500 transition-all uppercase tracking-widest">Enlist New Class</button>
    </div>
        ...
        {#each units as unit}
            <div class="bg-dark-translucent border border-white/5 p-8 rounded-3xl flex flex-col justify-between group hover:border-red-900/30 transition-all relative overflow-hidden">
                ...
                <div class="flex items-center gap-3 mt-8 relative z-10">
                    <button onclick={() => onDelete(unit.id)} class="w-12 h-12 rounded-xl bg-red-950/20 text-red-500 border border-red-900/30 flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-red-500 hover:text-white transition-all">✕</button>
                    <button onclick={() => onInspect(unit)} class="flex-grow py-4 bg-red-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-700 transition-all shadow-[0_0_20px_rgba(127,29,29,0.2)]">Tactical Calibration</button>
                </div>
            </div>
        {/each}
    </div>
